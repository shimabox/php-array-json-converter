...
`token_get_all()` を使う理由は、入力が「PHPとして実行可能か」ではなく「このツールが許可するPHP配列リテラルだけで構成されているか」を判定したいためです。

Array to JSONで受け取る値は、PHPの配列そのものではなくユーザーが入力した文字列です。その文字列をそのまま `json_encode()` すると、配列ではなく文字列としてJSON化されます。期待するJSON object / arrayを得るには、先に文字列をPHP配列リテラルとして安全に読み、PHPの値へ変換する必要があります。

正規表現や文字列分割だけでPHP配列リテラルを読むと、文字列中の `=>`, `,`, `[` などと構文上の記号を区別しづらくなります。一方で `eval()` や `include` はユーザー入力を実行してしまうため使いません。`token_get_all()` はPHPのlexerだけを使えるので、PHPの文字列・数値・記号をトークンとして安全に分解し、その後にパーサ側で `array()`, 関数呼び出し, 変数, 定数などの未対応構文を明示的に拒否できます。
